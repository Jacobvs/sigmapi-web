...
def user_can_edit(user=None, party_guest=None):
    ...
    """
    return (
        user.has_perm("PartyList.can_destroy_any_party_guest")
        or party_guest.addedBy == user
    )
...
def create(request, party_id):
    ...
    try:
        ...
        if form.is_valid():
            ...
            return HttpResponse(
                "Error adding guest: invalid guest format. " + "Contact webmaster.",
                status=500,
            )
    try:
...
def update_manual_delta(request, party_id):
    ...
    try:
        ...
        return HttpResponse(
            "Error: Invalid gender or count parameters." + " Contact webmaster.",
            status=500,
        )
    ...
    if gender == "M":
        ...
        return HttpResponse(
            "Error: Invalid gender parameter." + " Contact webmaster.", status=500
        )
...
@csrf_exempt
def sign_in(
    request, party_id, party_guest_id  # pylint: disable=unused-argument
