...
def modify_party_count(request, party_id):
    ...
    try:
        ...
        if not party.is_list_closed():
            return HttpResponse(
                "Cannot modify list count until the party starts", status=403
            )
        ...
        return JsonResponse({"party": party.to_json()})
...
def sign_out(request, party_id, party_guest_id):
    ...
    if not party.is_list_closed():
        return HttpResponse(
            "Can't sign out guests before the party starts.", status=403
        )
...
def destroy_guest(request, party_id, party_guest_id):
    ...
    if party.is_list_closed():
        return HttpResponse(
            "You cannot delete a guest after the party list closes.", status=403
        )
    ...
    try:
        ...
        return HttpResponse(
            "The guest you are trying to delete does not exist.", status=404
        )

    if guest.added_by == request.user or request.user.has_perm(
        "PartyListV2.can_destroy_any_party_guest"
    ):
        guest.delete()
...
def get_details(request, party_id):
    ...
    return JsonResponse({"party": requested_party.to_json()})
...
def get_guests(request, party_id):
    ...
    guests = (
        PartyGuest.objects.filter(party__id=party_id)
        .order_by(Lower("name"))
        .only("_cached_json")
    )
...
def get_delta_guests(request, party_id, update_counter):
    ...
    guest_ids = (
        PartyGuest.objects.filter(party__id=party_id).values_list("id", flat=True).all()
    )
    response = {
...
def get_permissions(request, party_id):
    ...
    response = {
        "permissions": {
            "canRemoveAnyGuest": request.user.has_perm(
                "PartyListV2.can_destroy_any_party_guest"
            ),
...
def create_guest(request, party_id):
    ...
    exists = (
        PartyGuest.objects.filter(
            name__iexact=guest_name, party=party, gender=guest_gender
        ).count()
        > 0
    )
    if exists:
    ...
    if (
        selected_brother_username is not None
        and not selected_brother_username.strip() == ""
    ):
        try:
    ...
    if not party.is_list_closed():
        ...
        if selected_brother is not None and selected_brother.id == request.user.id:
            return HttpResponse(
                "You cannot borrow an invite from yourself (nice try).", status=403
            )
        ...
        if selected_brother is None:
            return HttpResponse(
                "You must list a vouching brother when the list is in party mode.",
                status=400,
            )
        ...
        if party.user_reached_vouching_limit(selected_brother) and not risk_approval:
            return HttpResponse(
                "The vouching brother has reached their vouch limit, you will need the risk manager"
                " or president to authorize this addition.",
                status=403,
            )

        if not request.user.has_perm("PartyListV2.door_access"):
            return HttpResponse(
                "You do not have permission to create guests after the list has closed.",
                status=403,
            )
...
def __check_invite_limit(
    ...
    if preparty_access:
        ...
        if party.has_preparty_invite_limits:
            if selected_brother is None and party.user_reached_preparty_limit(
                request.user
            ):
                return HttpResponse(
                    "You have reached your pre-party invite limit.", status=403
                )
            if selected_brother is not None and party.user_reached_preparty_limit(
                selected_brother
            ):
                return HttpResponse(
                    "The brother you are trying to borrow invites from has "
                    "reached their pre-party invite limit.",
                    status=403,
                )
        if party.has_party_invite_limits:
            if selected_brother is None and party.user_reached_party_limit(
                request.user
            ):
                return HttpResponse(
                    "You have pre-party invites left, but your total "
                    "amount of guests has reached the limit for this party. Try "
                    "removing a regular guest before adding another to pre-party.",
                    status=403,
                )
            if selected_brother is not None:
                ...
                if party.user_reached_preparty_limit(selected_brother):
                    return HttpResponse(
                        "The brother you are trying to borrow invites"
                        " from has reached their pre-party invite limit.",
                        status=403,
                    )
    if party.has_party_invite_limits:
...
def export_list(_request, party_id):
    ...
    """
    requested_party = Party.objects.get(pk=party_id)
    party_guests = PartyGuest.objects.filter(party__exact=requested_party).order_by(
        "name"
    )
...
def get_counts_history(request, party_id):
    ...
    return JsonResponse({"entries": [entry.to_json() for entry in entries]})
