...
def users(request):
    ...
    gradstudents = (
        gradstudents.filter(
            userinfo__graduationYear__lt=senior_year, groups__name="Brothers"
        )
    ...
    seniors = (
        seniors.filter(userinfo__graduationYear=senior_year, groups__name="Brothers")
        .prefetch_related("userinfo")
        .order_by("last_name")
    )
    ...
    juniors = (
        User.objects.filter(
            userinfo__graduationYear=(senior_year + 1), groups__name="Brothers"
        )
        .prefetch_related("userinfo")
        .order_by("last_name")
    )
    ...
    sophomores = (
        User.objects.filter(
            userinfo__graduationYear=(senior_year + 2), groups__name="Brothers"
        )
        .prefetch_related("userinfo")
        .order_by("last_name")
    )
    ...
    freshmen = (
        User.objects.filter(
            userinfo__graduationYear=(senior_year + 3), groups__name="Brothers"
        )
        .prefetch_related("userinfo")
        .order_by("last_name")
    )
    ...
    sweethearts = (
        User.objects.filter(
            userinfo__graduationYear__gte=senior_year, groups__name="Sweethearts"
        )
        .prefetch_related("userinfo")
        .order_by("last_name")
    )
    ...
    context = {
        ...
        "brother_groups": [
            ...
            {"group_title": "Seniors", "brothers": seniors, "count": len(seniors)},
            {"group_title": "Juniors", "brothers": juniors, "count": len(juniors)},
            {
                "group_title": "Sophomores",
                "brothers": sophomores,
                "count": len(sophomores),
            },
            {"group_title": "Freshmen", "brothers": freshmen, "count": len(freshmen)},
            {
...
def family_tree_dashboard(request):
    ...
    user_accounts = (
        User.objects.filter(groups__name__in=["Brothers", "Alumni"])
...
def get_tree_json(request):
    ...
    root_node = {"name": "Sigma Pi Gamma Iota", "children": []}
    ...
    return JsonResponse({"tree": root_node})
    ...
    if request.method == "POST":
        ...
        if add_type == "SINGLE":
            ...
            try:
                ...
                context["message"].append("Error adding " + to_add + ": " + str(err))
            else:
...
def edit_user(request, user):
    ...
    context = {"requested_user": requested_user, "form": form, "error": form.errors}
    if message:
...
def profile_image(rel_path):
    ...
    """
    return "/content/" + rel_path if rel_path else "/static/img/user-placeholder.png"
